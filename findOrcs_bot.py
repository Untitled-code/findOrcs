...
def get_keywords(QUERY_LIST_FILE):
...
def avoid_duplicates(output_file):
    """Search in outputfile all ready parsed names and skip it"""
...
class CrawlerBingo:
    ...
    def __init__(self, searchSite, searchRequest, relation, searchingRequest):
        self.searchSite = searchSite
        self.searchRequest = searchRequest
        self.searchingRequest = searchingRequest
...
def main():
    ...
    global sites
    # sites = ["https://mil.ru/"]
    sites = ["https://mil.ru/", "http://publication.pravo.gov.ru/", "http://www.kremlin.ru/", "http://prezident.org/",
             "https://ria.ru/", "https://lenta.ru/", "https://ren.tv/", "https://www.soldati-russian.ru/",
             "https://tsargrad.tv/", "https://tvzvezda.ru/", "https://tass.ru/", "https://myrotvorets.center/",
             "https://ingushetia.ru/"]
    global OUTPUT_FILE
    OUTPUT_FILE = Path(f'./output.csv')
    cyrilNames = get_keywords(QUERY_LIST_FILE)
    ...
    for searchingRequest in range(len(searchingRequests)):
        ...
        if keywords[searchingRequest] not in results:
            ...
            for site in range(len(sites)):
                print(f'Searching... {keywords[searchingRequest]} on site {sites[site]} with {searchingRequest}')
                crawlerBingo = CrawlerBingo(sites[site], searchingRequests[searchingRequest], relation, searchingRequest)
                crawlerBingo.parse()
        else:
